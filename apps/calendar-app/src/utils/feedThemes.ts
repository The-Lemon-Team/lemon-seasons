import { Feed } from '@lenta/shared';

export interface FeedThemeConfig {
  slug: string;
  title: string;
  shortTitle: string;
  tagline: string;
  category: 'media' | 'engineering' | 'product' | 'design' | 'operations' | 'general';
  accentColor: string;
  gradient: string;
  borderAccent: string;
  bgLight: string;
  badgeBg: string;
  emoji: string;
  iconName: string;
}

export const KNOWN_FEED_THEMES: Record<string, FeedThemeConfig> = {
  'mcu-radar': {
    slug: 'mcu-radar',
    title: 'Marvel Cinematic Universe',
    shortTitle: 'MCU Radar',
    tagline: 'Film releases, hero arcs & multiverse phases',
    category: 'media',
    accentColor: '#f87171', // Red/Amber
    gradient: 'from-red-500/20 via-amber-500/10 to-transparent',
    borderAccent: '#ef4444',
    bgLight: 'rgba(239, 68, 68, 0.12)',
    badgeBg: 'rgba(239, 68, 68, 0.25)',
    emoji: '🎬',
    iconName: 'Film',
  },
  'tech-strategy': {
    slug: 'tech-strategy',
    title: 'Technical Architecture & Strategy',
    shortTitle: 'Tech Strategy',
    tagline: 'System designs, RFCs & engineering roadmaps',
    category: 'engineering',
    accentColor: '#38bdf8', // Sky/Cyan
    gradient: 'from-sky-500/20 via-cyan-500/10 to-transparent',
    borderAccent: '#0ea5e9',
    bgLight: 'rgba(14, 165, 233, 0.12)',
    badgeBg: 'rgba(14, 165, 233, 0.25)',
    emoji: '⚡',
    iconName: 'Zap',
  },
  'product-milestones': {
    slug: 'product-milestones',
    title: 'Product Milestones',
    shortTitle: 'Product Goals',
    tagline: 'Quarterly OKRs, features & user experience',
    category: 'product',
    accentColor: '#e5e971', // Lemon Gold
    gradient: 'from-[#c9cd58]/20 via-[#535600]/10 to-transparent',
    borderAccent: '#c9cd58',
    bgLight: 'rgba(201, 205, 88, 0.12)',
    badgeBg: 'rgba(201, 205, 88, 0.25)',
    emoji: '🎯',
    iconName: 'Target',
  },
  'design-systems': {
    slug: 'design-systems',
    title: 'Design Systems & UI',
    shortTitle: 'Design UI',
    tagline: 'Lenta tokens, components & UX patterns',
    category: 'design',
    accentColor: '#c084fc', // Purple/Violet
    gradient: 'from-purple-500/20 via-fuchsia-500/10 to-transparent',
    borderAccent: '#a855f7',
    bgLight: 'rgba(168, 85, 247, 0.12)',
    badgeBg: 'rgba(168, 85, 247, 0.25)',
    emoji: '🎨',
    iconName: 'Palette',
  },
  'devops-infra': {
    slug: 'devops-infra',
    title: 'DevOps & Infrastructure',
    shortTitle: 'DevOps & Cloud',
    tagline: 'Uptime ops, migrations & cloud deployments',
    category: 'operations',
    accentColor: '#34d399', // Emerald/Teal
    gradient: 'from-emerald-500/20 via-teal-500/10 to-transparent',
    borderAccent: '#10b981',
    bgLight: 'rgba(16, 185, 129, 0.12)',
    badgeBg: 'rgba(16, 185, 129, 0.25)',
    emoji: '🛠️',
    iconName: 'Server',
  },
  'russian-holidays': {
    slug: 'russian-holidays',
    title: 'Русские праздники',
    shortTitle: 'Праздники РФ',
    tagline: 'Государственные торжества и Дни воинской славы России',
    category: 'general',
    accentColor: '#60a5fa', // Blue/Red
    gradient: 'from-blue-500/20 via-red-500/10 to-transparent',
    borderAccent: '#3b82f6',
    bgLight: 'rgba(59, 130, 246, 0.12)',
    badgeBg: 'rgba(59, 130, 246, 0.25)',
    emoji: '🇷🇺',
    iconName: 'Flag',
  },
  'christian-holidays': {
    slug: 'christian-holidays',
    title: 'Христианские праздники',
    shortTitle: 'Пасха & Праздники',
    tagline: 'Двунадесятые праздники, Пасха и церковный год 2026',
    category: 'general',
    accentColor: '#fbbf24', // Gold / Amber
    gradient: 'from-amber-500/20 via-yellow-500/10 to-transparent',
    borderAccent: '#f59e0b',
    bgLight: 'rgba(245, 158, 11, 0.12)',
    badgeBg: 'rgba(245, 158, 11, 0.25)',
    emoji: '☦️',
    iconName: 'Sparkles',
  },
  'orthodox-holidays': {
    slug: 'orthodox-holidays',
    title: 'Православный календарь',
    shortTitle: 'Православие',
    tagline: 'Пасха Христова, двунадесятые и великие праздники, посты и дни памяти святых 2026',
    category: 'general',
    accentColor: '#f59e0b', // Amber/Gold
    gradient: 'from-amber-500/20 via-yellow-500/10 to-transparent',
    borderAccent: '#d97706',
    bgLight: 'rgba(245, 158, 11, 0.12)',
    badgeBg: 'rgba(245, 158, 11, 0.25)',
    emoji: '☦️',
    iconName: 'Sparkles',
  },
  'catholic-holidays': {
    slug: 'catholic-holidays',
    title: 'Католический календарь',
    shortTitle: 'Католицизм',
    tagline: 'Торжества латинского обряда, Пасхальное триденствие, Пепельная среда и Адвент 2026',
    category: 'general',
    accentColor: '#8b5cf6', // Violet
    gradient: 'from-purple-500/20 via-indigo-500/10 to-transparent',
    borderAccent: '#7c3aed',
    bgLight: 'rgba(139, 92, 246, 0.12)',
    badgeBg: 'rgba(139, 92, 246, 0.25)',
    emoji: '✝️',
    iconName: 'Bookmark',
  },
  'islamic-holidays': {
    slug: 'islamic-holidays',
    title: 'Исламский календарь',
    shortTitle: 'Ислам (Хиджра)',
    tagline: 'Священный месяц Рамадан, Ночь Аль-Кадр, Ураза-байрам, Курбан-байрам и мусульманские праздники',
    category: 'general',
    accentColor: '#10b981', // Emerald / Islamic green
    gradient: 'from-emerald-500/20 via-teal-500/10 to-transparent',
    borderAccent: '#059669',
    bgLight: 'rgba(16, 185, 129, 0.12)',
    badgeBg: 'rgba(16, 185, 129, 0.25)',
    emoji: '☪️',
    iconName: 'Moon',
  },
  'world-religions': {
    slug: 'world-religions',
    title: 'Мировые религии',
    shortTitle: 'Мировые религии',
    tagline: 'Межконфессиональный календарь: Ислам, Иудаизм, Буддизм и мировые духовные традиции',
    category: 'general',
    accentColor: '#06b6d4', // Cyan
    gradient: 'from-cyan-500/20 via-sky-500/10 to-transparent',
    borderAccent: '#0891b2',
    bgLight: 'rgba(6, 182, 212, 0.12)',
    badgeBg: 'rgba(6, 182, 212, 0.25)',
    emoji: '🕊️',
    iconName: 'Globe',
  },
  'russian-official': {
    slug: 'russian-official',
    title: 'Государственные праздники России',
    shortTitle: 'Госпраздники РФ',
    tagline: 'Официальные нерабочие праздничные дни (ТК РФ ст. 112), памятные даты и культурные события',
    category: 'general',
    accentColor: '#ef4444', // Red
    gradient: 'from-red-500/20 via-rose-500/10 to-transparent',
    borderAccent: '#dc2626',
    bgLight: 'rgba(239, 68, 68, 0.12)',
    badgeBg: 'rgba(239, 68, 68, 0.25)',
    emoji: '🇷🇺',
    iconName: 'Flag',
  },
  'russian-military': {
    slug: 'russian-military',
    title: 'Дни воинской славы и военные праздники РФ',
    shortTitle: 'Военные даты РФ',
    tagline: 'Дни воинской славы России (32-ФЗ), рода войск Вооруженных Сил и памятные сражения Отечества',
    category: 'general',
    accentColor: '#eab308', // Gold/Bronze
    gradient: 'from-amber-600/20 via-yellow-600/10 to-transparent',
    borderAccent: '#ca8a04',
    bgLight: 'rgba(234, 179, 8, 0.12)',
    badgeBg: 'rgba(234, 179, 8, 0.25)',
    emoji: '🎖️',
    iconName: 'Shield',
  },
  'world-holidays': {
    slug: 'world-holidays',
    title: 'Праздники стран мира',
    shortTitle: 'Праздники мира',
    tagline: 'Знаменитые праздники и торжества стран мира: Китайский Новый год, День взятия Бастилии, День благодарения',
    category: 'general',
    accentColor: '#3b82f6', // Blue
    gradient: 'from-blue-500/20 via-indigo-500/10 to-transparent',
    borderAccent: '#2563eb',
    bgLight: 'rgba(59, 130, 246, 0.12)',
    badgeBg: 'rgba(59, 130, 246, 0.25)',
    emoji: '🌍',
    iconName: 'Globe',
  },
  'politics-2026': {
    slug: 'politics-2026',
    title: 'Политика 2026',
    shortTitle: 'Политика 2026',
    tagline: 'Международные саммиты, выборы и геополитика 2026 года',
    category: 'general',
    accentColor: '#f43f5e', // Rose/Indigo
    gradient: 'from-rose-500/20 via-indigo-500/10 to-transparent',
    borderAccent: '#f43f5e',
    bgLight: 'rgba(244, 63, 94, 0.12)',
    badgeBg: 'rgba(244, 63, 94, 0.25)',
    emoji: '🌐',
    iconName: 'Globe',
  },
};

// Curated dynamic palette for unmapped feeds
const DYNAMIC_PALETTES = [
  { accent: '#fb923c', border: '#f97316', emoji: '🔥', icon: 'Flame', bg: 'rgba(249, 115, 22, 0.15)' },
  { accent: '#22d3ee', border: '#06b6d4', emoji: '🌐', icon: 'Globe', bg: 'rgba(6, 182, 212, 0.15)' },
  { accent: '#a78bfa', border: '#8b5cf6', emoji: '✨', icon: 'Sparkles', bg: 'rgba(139, 92, 246, 0.15)' },
  { accent: '#4ade80', border: '#22c55e', emoji: '🌱', icon: 'Radio', bg: 'rgba(34, 197, 94, 0.15)' },
  { accent: '#f472b6', border: '#ec4899', emoji: '📰', icon: 'Newspaper', bg: 'rgba(236, 72, 153, 0.15)' },
];

export function getFeedTheme(slug?: string, title?: string): FeedThemeConfig {
  if (slug && KNOWN_FEED_THEMES[slug]) {
    return KNOWN_FEED_THEMES[slug];
  }

  // Deterministic fallback based on slug hash
  const key = slug || title || 'default';
  let hash = 0;
  for (let i = 0; i < key.length; i++) {
    hash = (hash << 5) - hash + key.charCodeAt(i);
    hash |= 0;
  }
  const palette = DYNAMIC_PALETTES[Math.abs(hash) % DYNAMIC_PALETTES.length];

  return {
    slug: slug || 'custom',
    title: title || 'Custom Feed',
    shortTitle: title ? (title.length > 16 ? title.slice(0, 16) + '...' : title) : 'Custom Feed',
    tagline: 'Custom chronological news stream',
    category: 'general',
    accentColor: palette.accent,
    gradient: 'from-amber-500/20 to-transparent',
    borderAccent: palette.border,
    bgLight: palette.bg,
    badgeBg: palette.bg,
    emoji: palette.emoji,
    iconName: palette.icon,
  };
}

export interface FeedPresetOption {
  id: string;
  name: string;
  emoji: string;
  description: string;
  slug?: string; // undefined means all channels
}

export const FEED_PRESET_OPTIONS: FeedPresetOption[] = [
  {
    id: 'all',
    name: 'All Channels',
    emoji: '📡',
    description: 'All news channels and editorial streams combined',
    slug: undefined,
  },
  {
    id: 'mcu-radar',
    name: 'MCU Radar',
    emoji: '🎬',
    description: 'Marvel MCU radar, film releases & phase timelines',
    slug: 'mcu-radar',
  },
  {
    id: 'tech-strategy',
    name: 'Tech Strategy',
    emoji: '⚡',
    description: 'Technical architecture, RFCs & engineering roadmaps',
    slug: 'tech-strategy',
  },
  {
    id: 'product-milestones',
    name: 'Product Goals',
    emoji: '🎯',
    description: 'Product goals, quarterly OKRs & user experience',
    slug: 'product-milestones',
  },
  {
    id: 'design-systems',
    name: 'Design UI',
    emoji: '🎨',
    description: 'Design UI tokens & component system',
    slug: 'design-systems',
  },
  {
    id: 'devops-infra',
    name: 'DevOps & Infra',
    emoji: '🛠️',
    description: 'DevOps, cloud infra & deployment logs',
    slug: 'devops-infra',
  },
  {
    id: 'russian-official',
    name: 'Праздники РФ (государственные)',
    emoji: '🇷🇺',
    description: 'Официальные нерабочие праздничные дни и культурные торжества России',
    slug: 'russian-official',
  },
  {
    id: 'russian-military',
    name: 'Дни воинской славы РФ',
    emoji: '🎖️',
    description: 'Дни воинской славы, рода войск и памятные даты России (32-ФЗ)',
    slug: 'russian-military',
  },
  {
    id: 'orthodox-holidays',
    name: 'Православный календарь',
    emoji: '☦️',
    description: 'Пасха Христова, двунадесятые и великие праздники, посты 2026',
    slug: 'orthodox-holidays',
  },
  {
    id: 'catholic-holidays',
    name: 'Католический календарь',
    emoji: '✝️',
    description: 'Торжества латинского обряда, Пасхальное триденствие и праздники 2026',
    slug: 'catholic-holidays',
  },
  {
    id: 'islamic-holidays',
    name: 'Исламский календарь',
    emoji: '☪️',
    description: 'Священный месяц Рамадан, Ночь Аль-Кадр, Ураза-байрам, Курбан-байрам',
    slug: 'islamic-holidays',
  },
  {
    id: 'world-religions',
    name: 'Мировые религии',
    emoji: '🕊️',
    description: 'Календарь мировых конфессий: Ислам, Иудаизм, Буддизм',
    slug: 'world-religions',
  },
  {
    id: 'world-holidays',
    name: 'Праздники стран мира',
    emoji: '🌍',
    description: 'Национальные и культурные праздники стран мира',
    slug: 'world-holidays',
  },
  {
    id: 'russian-holidays',
    name: 'Все русские праздники (сводный)',
    emoji: '🇷🇺',
    description: 'Государственные праздники и Дни воинской славы России',
    slug: 'russian-holidays',
  },
  {
    id: 'christian-holidays',
    name: 'Все христианские праздники (сводный)',
    emoji: '☦️',
    description: 'Двунадесятые праздники, Пасха Христова и посты 2026',
    slug: 'christian-holidays',
  },
  {
    id: 'politics-2026',
    name: 'Политика 2026',
    emoji: '🌐',
    description: 'Международные саммиты, выборы и геополитика 2026',
    slug: 'politics-2026',
  },
];

